<#
.SYNOPSIS
    Generate SBOM documentation over a set of revisions using ONE warm Windows LabVIEW container.

.DESCRIPTION
    Windows-container "batch" counterpart of run-sbom-windows-container.yml.
    ...
    then published with keep_files semantics into gh-pages sbom/<sha>/.
...
param(
    ...
    # 'sbom/<sha>/summary.json'). Used to skip done revisions.
    [string]$SkipListPath      = '',
...
if ($OutRoot -eq '') { $OutRoot = Join-Path $WorkspaceRoot 'ci-out\sbom-backfill' }
...
if (-not $Force -and $SkipListPath -ne '' -and (Test-Path $SkipListPath)) {
    foreach ($line in (Get-Content $SkipListPath)) {
        if ($line -match 'sbom/([0-9a-f]{7,40})/') { [void]$Done.Add($Matches[1]) }
    }
...
# (run-sbom.ps1 etc.) is mounted at C:\ops, and per-revision source
    ...
    foreach ($sha in $Commits) {
        ...
        try {
            ...
            & docker exec -e "GITHUB_SHA=$sha" $ContainerName powershell -NonInteractive -ExecutionPolicy Bypass `
                -File 'C:\ops\run-sbom.ps1' `
                -WorkspaceRoot "C:\wt\src-$sha" `
                -ReportDir     "$cOut" `
                -LabVIEWPath   $LabVIEWPath
            if ($LASTEXITCODE -ne 0) { Write-Warning "run-sbom returned $LASTEXITCODE for $short (continuing)." }
            ...
            # workflow does. run-sbom always emits a safety-net index.html, so this
            ...
            & python (Join-Path $OpsHost 'build-sbom-report.py') `
                --in         $reportHostDir `
                --out        $reportHostDir `
                --platform   windows `
                --sha        $sha `
...
Write-Host ""
Write-Host "=== SBOM backfill (Windows) complete: $processed documented, $skipped skipped ==="
exit 0
